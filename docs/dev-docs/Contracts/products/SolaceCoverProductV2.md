...
### maxRate
```solidity
  function maxRate() public returns (uint256 maxRateNum_, uint256 maxRateDenom_)
```
Gets the max rate.
...
### isSupportedChain
```solidity
  function isSupportedChain() public returns (bool status)
```
...
### numSupportedChains
```solidity
  function numSupportedChains() public returns (uint256 count)
```
...
### _getAsset
```solidity
  function _getAsset() internal returns (contract IERC20)
```
